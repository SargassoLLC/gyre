...
use gyre::channels::web::server::{GatewayState, start_server};
use gyre::channels::web::sse::SseManager;
use gyre::channels::web::ws::WsConnectionTracker;
use gyre::error::LlmError;
use gyre::llm::{
...
impl LlmProvider for MockLlmProvider {
    ...
    async fn complete(&self, req: CompletionRequest) -> Result<CompletionResponse, LlmError> {
        ...
        let user_msg = req
            ...
            .iter()
            .rev()
            .find(|m| m.role == gyre::llm::Role::User)
    ...
    async fn complete_with_tools(
        ...
        if let Some(tool) = req.tools.first() {
            Ok(ToolCompletionResponse {
                content: None,
                tool_calls: vec![gyre::llm::ToolCall {
...
async fn start_test_server() -> (SocketAddr, Arc<GatewayState>) {
    let state = Arc::new(GatewayState {
        ...
        skill_registry: None,
        skill_catalog: None,
        chat_rate_limiter: gyre::channels::web::server::RateLimiter::new(30, 60),
    });
...
async fn test_chat_completions_streaming() {
    ...
    assert_eq!(
        resp.headers()
            .get("x-gyre-streaming")
            .and_then(|v| v.to_str().ok()),
        Some("simulated"),
        "Expected x-gyre-streaming: simulated header"
    );
