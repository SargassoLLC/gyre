...
use gyre::agent::{Agent, AgentDeps};
use gyre::channels::{ChannelManager, IncomingMessage};
use gyre::config::AgentConfig;
use gyre::llm::LlmProvider;
use gyre::safety::SafetyLayer;
use gyre::tools::ToolRegistry;
...
struct TaskRunParams<'a> {
    ...
    additional_tools: &'a [Arc<dyn gyre::tools::Tool>],
}
...
impl BenchRunner {
    ...
    pub async fn run(
        ...
        if self.config.parallelism <= 1 {
            ...
            let shared_tools: Arc<[Arc<dyn gyre::tools::Tool>]> =
...
async fn run_task_isolated(params: TaskRunParams<'_>) -> TaskResult {
    ...
    let cost_guard = Arc::new(gyre::agent::cost_guard::CostGuard::new(
        gyre::agent::cost_guard::CostGuardConfig::default(),
    ));
    ...
    let deps = AgentDeps {
        ...
        skill_registry: None,
        skills_config: gyre::config::SkillsConfig::default(),
        hooks: Arc::new(gyre::hooks::HookRegistry::new()),
        cost_guard,
        cognitive: None,
        identity: None,
    };
