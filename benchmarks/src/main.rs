...
#[derive(Parser)]
#[command(name = "gyre-bench", about = "Gyre benchmarking harness")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| EnvFilter::new("gyre_bench=info,gyre=warn")),
        )
    ...
    match cli.command {
        ...
        Commands::Run {
            ...
            // Initialize Gyre LLM provider
            let gyre_config = gyre::Config::from_env().await.map_err(|e| {
                anyhow::anyhow!(
                    "Failed to load gyre config: {}. Make sure .env is configured.",
                    e
                )
            })?;

            let session =
                gyre::llm::create_session_manager(gyre::llm::SessionConfig::default()).await;

            let llm = gyre::llm::create_llm_provider(&gyre_config.llm, session)?;
            let safety = Arc::new(gyre::safety::SafetyLayer::new(&gyre_config.safety));
