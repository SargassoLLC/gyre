...
**Gyre** is a secure personal AI assistant that protects your data and expands its capabilities on the fly.
...
│   ├── session.rs      # Gyre session management
...
│
├── workspace/          # Persistent memory system (ambient-first)
...
DATABASE_URL=postgres://user:pass@localhost/gyre
LIBSQL_PATH=~/.gyre/gyre.db    # libSQL local path (default)
# LIBSQL_URL=libsql://xxx.turso.io    # Turso cloud (optional)
# LIBSQL_AUTH_TOKEN=xxx                # Required with LIBSQL_URL

# Gyre (required)
GYRE_SESSION_TOKEN=sess_...
GYRE_MODEL=claude-3-5-sonnet-20241022
GYRE_BASE_URL=https://api.getgyre.com
...
# Or use Gyre embeddings:
# EMBEDDING_PROVIDER=gyre
...
Uses the configured LLM provider (Anthropic, OpenAI, Ollama) which provides:
- Unified access to multiple models (OpenAI, Anthropic, etc.)
- User authentication via session tokens
- Usage tracking and billing through Gyre

Session tokens have the format `sess_xxx` (37 characters). They are authenticated against the Gyre auth service.

## Database

Gyre supports two database backends, selected at compile time via Cargo feature flags and at runtime via the `DATABASE_BACKEND` environment variable.
...
# libSQL (embedded)
LIBSQL_PATH=~/.gyre/gyre.db    # Default path
...
4. ~~**WIT bindgen integration**~~ - ✅ Completed: `src/tools/wasm/wit_schema.rs` auto-extracts description/schema from WASM modules at load time
...
- ✅ **Embeddings integration** - OpenAI and Gyre LLM providers wired to workspace for semantic search
...
- ✅ **WIT bindgen integration** - Auto-extract tool description/schema from WASM modules at load time (`src/tools/wasm/wit_schema.rs`)
...
5. Install with `gyre tool install path/to/tool.wasm`

See `tools-src/` for examples.

### WIT Schema Auto-Extraction

WASM tools self-describe via their WIT exports. At load time, the host instantiates the component
and calls `description()` and `schema()` to automatically extract:

- **Tool description** — returned by the `description()` export
- **Parameter schema** — JSON Schema returned by the `schema()` export
- **Parameter types** — parsed from the JSON Schema into WIT type mappings

This means you do NOT need to duplicate schema in `capabilities.json`. The capabilities file
only needs HTTP allowlists, secrets, and auth config. Schema is auto-extracted from the WASM binary.

**Priority chain:** explicit override (registration) > WIT extraction > placeholder defaults.

WIT type ↔ JSON Schema mapping:
| WIT Type | JSON Schema |
|----------|-------------|
| `string` | `{"type": "string"}` |
| `u32`, `s64`, etc. | `{"type": "integer"}` |
| `f32`, `f64` | `{"type": "number"}` |
| `bool` | `{"type": "boolean"}` |
...
When running `gyre tool auth <tool>`:
...
Both are first-class in the extension system (`gyre tool install` handles both), but they have different strengths.

**WASM Tools (Gyre native)**
...
- Cost: external process with full system access (no sandbox), manages own credentials, Gyre can't prevent leaks
...
```bash
# Verbose logging
RUST_LOG=gyre=trace cargo run

# Just the agent module
RUST_LOG=gyre::agent=debug cargo run

# With HTTP request logging
RUST_LOG=gyre=debug,tower_http=debug cargo run
```
...
The workspace provides persistent memory for agents with a flexible filesystem-like structure.
