...
//!
//! Stores user preferences in ~/.gyre/settings.json.
...
// Re-export multi-agent config types so consumers can use `settings::MultiAgentSettings` etc.
pub use crate::setup::config_schema::{
    AgentDefinition, ChannelBinding, GatewaySettings, MultiAgentSettings,
};
...
fn default_sandbox_image() -> String {
    "ghcr.io/sargassollc/gyre-sandbox:latest".to_string()
}
...
impl Default for CircuitBreakerSettings {
    fn default() -> Self {
        Self {
            ...
            reset_timeout_secs: default_cb_reset_timeout(),
        }
    }
}
...
pub struct ResponseCacheSettings {
    /// Whether response caching is enabled (default: false).
    #[serde(default)]
    pub enabled: bool,
...
pub struct FailoverSettings {
    /// Whether failover is enabled (default: false).
    #[serde(default)]
    pub enabled: bool,

    /// Fallback LLM backend (e.g. "ollama", "openai").
    #[serde(default)]
    ...
    /// Fallback base URL (for ollama/openai_compatible).
    #[serde(default)]
    ...
    /// Cooldown duration in seconds after failover activates.
...
impl Default for FailoverSettings {
    fn default() -> Self {
        Self {
            ...
            cooldown_secs: default_failover_cooldown(),
        }
    }
}
...
impl Settings {
    ...
    pub fn from_db_map(map: &std::collections::HashMap<String, serde_json::Value>) -> Self {
        ...
        // data (e.g. gyre.session_token). Skip keys that don't correspond to
        ...
        for (key, value) in map {
            ...
            match settings.set(key, &value_str) {
                ...
                // data (e.g. gyre.session_token). Silently skip unknown paths.
    ...
    /// Gyre configuration directory (~/.gyre/).
    ...
    /// Legacy config.env path (~/.gyre/config.env).
    pub fn config_env_path() -> PathBuf {
        Self::gyre_dir().join("config.env")
    }

    /// Get the default settings file path (~/.gyre/settings.json).
    pub fn default_path() -> std::path::PathBuf {
        Self::gyre_dir().join("settings.json")
    }
    ...
    /// Default TOML config file path (~/.gyre/config.toml).
    pub fn default_toml_path() -> PathBuf {
        Self::gyre_dir().join("config.toml")
    }
    ...
    pub fn save_toml(&self, path: &std::path::Path) -> Result<(), String> {
        ...
        let content = format!(
            "# Gyre configuration file.\n\
             #\n\
             # Priority: env var > this file > database settings > defaults.\n\
             # Uncomment and edit values to override defaults.\n\
             # Run `gyre config init` to regenerate this file.\n\
             #\n\
             # Documentation: https://github.com/gyre-ai/gyre\n\
             \n\
             {raw}"
        );
...
mod tests {
    ...
    fn test_get_setting() {
        ...
        assert_eq!(settings.get("agent.name"), Some("gyre".to_string()));
        assert_eq!(
    ...
    fn test_reset_setting() {
        ...
        assert_eq!(settings.agent.name, "gyre");
    }
    ...
    fn toml_header_comment_present() {
        ...
        assert!(content.starts_with("# Gyre configuration file."));
    ...
    #[test]
    fn default_toml_path_under_gyre() {
        let path = Settings::default_toml_path();
        assert!(path.to_string_lossy().contains(".gyre"));
