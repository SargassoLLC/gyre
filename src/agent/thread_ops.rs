    ...
    pub(super) async fn process_user_input(
        ...
        match result {
            Ok(AgenticLoopResult::Response(response)) => {
                ...
                // Post-turn: extract and store memories from assistant response
                if let Some(ref cog_ctx) = self.deps.cognitive {
                    if let Ok(ms) = cog_ctx.memory_stream.lock() {
                        cognitive::auto_store_memories(
                            &ms,
                            &response,
                            &cognitive::AutoMemoryConfig::default(),
                        );

                        // Emit agent status after cognitive post-turn
                        let memory_count = ms.count();
                        drop(ms); // release lock before async call
                        let queue_depth = cognitive::curiosity::ResearchQueue::open_for_hermit_box(
                            &cog_ctx.hermit_box,
                        )
                        .ok()
                        ...
                        let _ = self
                            ...
                            .send_status(
                                ...
                                StatusUpdate::AgentStatus {
                                    agent_id: cog_ctx.hermit_box.agent_id.clone(),
                                    memory_count,
                                    queue_depth,
                                },
                                &message.metadata,
