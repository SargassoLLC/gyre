...
impl SessionManager {
    ...
    pub async fn prune_stale_sessions(&self, max_idle: std::time::Duration) -> usize {
        ...
        // Collect per-session thread IDs for the SessionEnd event so that
        // downstream hooks (e.g. session-ingestion) can query persisted messages.
        let session_thread_ids: HashMap<String, Vec<Uuid>> = {
            let sessions = self.sessions.read().await;
            stale_sessions
                .iter()
                .filter_map(|(user_id, _)| {
                    ...
                    let sess = session.try_lock().ok()?;
                    Some((user_id.clone(), sess.threads.keys().cloned().collect()))
                })
                .collect()
        };
        ...
        if let Some(ref hooks) = self.hooks {
            for (user_id, session_id) in &stale_sessions {
                ...
                let tids = session_thread_ids.get(user_id).cloned().unwrap_or_default();
                tokio::spawn(async move {
                    use crate::hooks::HookEvent;
