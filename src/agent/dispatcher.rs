...
impl Agent {
    ...
    pub(super) async fn run_agentic_loop(
        ...
        // Build the composite system prompt: identity + cognitive + workspace
        let mut composite_prompt_parts: Vec<String> = Vec::new();

        // 1. Agent identity from HermitBox (Task 1: wire identity into system prompt)
        if let Some(ref id_files) = self.deps.identity {
            let block = id_files.system_prompt_block();
            if !block.is_empty() {
                composite_prompt_parts.push(block);
            }
        }

        // 2. Cognitive context — memories, knowledge, axioms (Task 2: pre-turn)
        if let Some(ref cog_ctx) = self.deps.cognitive {
            let cog_prefix = cognitive::turn::prepare_cognitive_context(cog_ctx, &message.content);
            let formatted = cognitive::turn::format_cognitive_prefix(&cog_prefix);
            if !formatted.is_empty() {
                composite_prompt_parts.push(formatted);
            }
        }
        ...
        let final_system_prompt = if composite_prompt_parts.is_empty() {
            None
        } else {
            Some(composite_prompt_parts.join("\n\n---\n\n"))
        };

        let mut reasoning = Reasoning::new(self.llm().clone(), self.safety().clone());
        if let Some(prompt) = final_system_prompt {
