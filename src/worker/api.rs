...
//!
//! Every request includes a bearer token from `GYRE_WORKER_TOKEN` env var.
...
impl WorkerHttpClient {
    ...
    ///
    /// Reads `GYRE_WORKER_TOKEN` from the environment.
    pub fn from_env(orchestrator_url: String, job_id: Uuid) -> Result<Self, WorkerError> {
        let token = std::env::var("GYRE_WORKER_TOKEN").map_err(|_| WorkerError::MissingToken)?;
