//! Gyre SKILL.md-based skills system.
//!
...
pub enum SkillSource {
    ...
    Workspace(PathBuf),
    /// User skills directory (~/.gyre/skills/).
    User(PathBuf),
...
pub struct SkillMetadata {
    /// Gyre-specific skill metadata.
    #[serde(default)]
    pub gyre: Option<GyreSkillMeta>,
}

/// Gyre-specific skill metadata.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct GyreSkillMeta {
...
"#;
    ...
    #[test]
    fn test_parse_gyre_skill_metadata() {
        let yaml = r#"
name: test-skill
metadata:
  gyre:
...
"#;
        ...
        let gyre_meta = meta.gyre.unwrap();
        assert_eq!(gyre_meta.requires.bins, vec!["vale"]);
        assert_eq!(gyre_meta.requires.env, vec!["VALE_CONFIG"]);
        assert_eq!(gyre_meta.requires.config, vec!["/etc/vale.ini"]);
    }
