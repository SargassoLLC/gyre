...
/// Get the history file path (~/.gyre/history).
    ...
    async fn start(&self) -> Result<MessageStream, ChannelError> {
        ...
        std::thread::spawn(move || {
            ...
            if !suppress_banner.load(Ordering::Relaxed) {
                println!("\x1b[1mGyre\x1b[0m  /help for commands, /quit to exit");
                println!();
            }
    ...
    async fn send_status(
        ...
        match status {
            ...
            StatusUpdate::AgentStatus {
                ...
                if debug {
                    eprintln!(
                        "  \x1b[36m[agent:{agent_id}]\x1b[0m memories:{memory_count} queue:{queue_depth}"
                    );
                }
            }
        }
