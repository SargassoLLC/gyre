...
function authenticate() {
  ...
  apiFetch('/api/chat/threads')
    .then(() => {
      sessionStorage.setItem('gyre_token', token);
    ...
    .catch(() => {
      sessionStorage.removeItem('gyre_token');
...
(function autoAuth() {
  ...
  const saved = sessionStorage.getItem('gyre_token');
  if (saved) {
...
function connectSSE() {
  ...
  // Brain graph update from cognitive layer
  eventSource.addEventListener('brain_update', (e) => {
    const data = JSON.parse(e.data);
    if (data.fired_nodes) fireBrainNodes(data.fired_nodes);
  });
}
...
// =============================================================================
// Brain Panel — D3.js force-directed neural graph
// =============================================================================
...
function initBrainGraph() {
  ...
  svg.call(
    d3.zoom()
      .scaleExtent([0.3, 3])
      .on('zoom', (event) => brainG.attr('transform', event.transform))
  );

  brainSim = d3.forceSimulation()
  ...
  brainG.append('g').attr('class', 'edges');
  brainG.append('g').attr('class', 'nodes');
}

function tickBrain() {
  brainG.select('.edges').selectAll('line')
    .attr('x1', d => d.source.x)
    .attr('y1', d => d.source.y)
    .attr('x2', d => d.target.x)
    .attr('y2', d => d.target.y);

  brainG.select('.nodes').selectAll('.brain-node')
    .attr('transform', d => 'translate(' + d.x + ',' + d.y + ')');
}

function updateBrainGraph(data) {
  if (!brainG) return;

  brainNodes = data.nodes.map(n => {
    // Preserve positions from previous run
    const existing = brainNodes.find(e => e.id === n.id);
    return Object.assign({}, n, existing ? { x: existing.x, y: existing.y } : {});
  });
  brainEdges = data.edges.map(e => ({ source: e.source, target: e.target, weight: e.weight }));

  // Edges
  const link = brainG.select('.edges').selectAll('line')
    .data(brainEdges, d => d.source + '-' + d.target);
  link.exit().remove();
  ...
  // Nodes
  const node = brainG.select('.nodes').selectAll('.brain-node')
  ...
  enter.append('circle')
    .attr('r', d => 6 + d.importance * 2)
    .attr('fill', d => BRAIN_COLORS[d.layer] || '#4a9eff')
    .attr('opacity', d => 0.4 + d.activation * 0.6);

  enter.append('text')
    .attr('dy', d => -(8 + d.importance * 2))
    .text(d => d.label);
  ...
  brainSim.nodes(brainNodes);
  brainSim.force('link').links(brainEdges);
  brainSim.alpha(0.3).restart();
}

function fireBrainNodes(nodeLabels) {
  if (!brainG || !nodeLabels || !nodeLabels.length) return;
  const labels = new Set(nodeLabels);

  brainG.select('.nodes').selectAll('.brain-node')
    .filter(d => labels.has(d.label))
    .each(function(d) {
      const g = d3.select(this);
      const color = BRAIN_COLORS[d.layer] || '#4a9eff';
      g.select('circle')
        .transition().duration(200)
        .attr('r', (6 + d.importance * 2) * 1.8)
        .style('filter', 'drop-shadow(0 0 6px ' + color + ')')
        .transition().duration(1300)
        .attr('r', 6 + d.importance * 2)
        .style('filter', null);
    });
}

function dragStart(event, d) {
  if (!event.active) brainSim.alphaTarget(0.3).restart();
  d.fx = d.x;
  d.fy = d.y;
}
...
function dragEnd(event, d) {
  if (!event.active) brainSim.alphaTarget(0);
  d.fx = null;
  d.fy = null;
}
...
async function pollBrainStatus() {
  try {
    ...
    if (res.ok) {
      ...
      updateBrainGraph(data);
      document.getElementById('brain-agent-name').textContent = data.agent_id;
      document.getElementById('brain-stats').textContent =
        'memories: ' + data.memory_count +
        ' | kg: ' + data.kg_entity_count +
