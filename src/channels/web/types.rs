...
pub enum SseEvent {
    ...
    /// Cognitive agent status (sent when cognitive layer is active).
    ...
    /// Brain graph update (fired nodes after a turn).
    #[serde(rename = "brain_update")]
    BrainUpdate {
        fired_nodes: Vec<String>,
        #[serde(skip_serializing_if = "Option::is_none")]
...
impl WsServerMessage {
    ...
    pub fn from_sse_event(event: &SseEvent) -> Self {
        let event_type = match event {
            ...
            SseEvent::AgentStatus { .. } => "agent_status",
            SseEvent::BrainUpdate { .. } => "brain_update",
        };
