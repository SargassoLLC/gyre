...
struct ChannelStoreData {
    ...
    /// Shared pending OAuth map for registering OAuth flows.
    pending_oauth: Option<
        Arc<
            tokio::sync::Mutex<HashMap<String, crate::channels::wasm::router::PendingOAuthRequest>>,
        >,
    >,
    /// Shared completed OAuth map for polling flow results by nonce.
    completed_oauth: Option<
        Arc<
            tokio::sync::Mutex<
                HashMap<String, crate::channels::wasm::router::OAuthCompletionResult>,
            >,
        >,
    >,
    /// Redirect URI base for OAuth callbacks (e.g., "http://localhost:3456").
...
impl ChannelStoreData {
    fn new(
        ...
        Self {
            ...
            http_runtime: None,
            pending_oauth: None,
            completed_oauth: None,
            oauth_redirect_base: None,
        }
    }
...
impl near::agent::channel_host::Host for ChannelStoreData {
    ...
    fn oauth_start(
        &mut self,
        req: near::agent::channel_host::OauthStartRequest,
    ) -> Result<near::agent::channel_host::OauthStartResult, String> {
        use crate::channels::wasm::router::PendingOAuthRequest;

        let pending_map = self
            .pending_oauth
            .as_ref()
            .ok_or_else(|| "OAuth not available: no pending OAuth store configured".to_string())?;

        let redirect_base = self
            .oauth_redirect_base
            .as_ref()
            .ok_or_else(|| "OAuth not available: no redirect base configured".to_string())?;

        let redirect_uri = format!("{}/oauth/callback", redirect_base);
        ...
        if !req.scopes.is_empty() {
            auth_url.push_str(&format!("&scope={}", urlencoding::encode(&req.scopes)));
        }
        // Append extra params if provided
        if !req.extra_params_json.is_empty()
            && req.extra_params_json != "{}"
            && let Ok(extra) =
                serde_json::from_str::<HashMap<String, String>>(&req.extra_params_json)
        {
            for (k, v) in &extra {
        ...
        let pending_request = PendingOAuthRequest {
            channel_id: self.host_state.channel_name().to_string(),
            token_url: req.token_url,
            client_id: req.client_id,
            client_secret_name: req.client_secret_name,
            redirect_uri,
            secret_name: req.secret_name,
            created_at: std::time::Instant::now(),
        };

        // Register in the shared pending map (blocking since WASM is sync)
        let map = pending_map.clone();
        let nonce_clone = nonce.clone();
        let rt = self.http_runtime.get_or_insert_with(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("failed to create tokio runtime for OAuth")
        });
        rt.block_on(async {
            ...
            guard.retain(|_, req| {
                req.created_at.elapsed().as_secs()
                    < crate::channels::wasm::router::PENDING_OAUTH_MAX_AGE_SECS
            });
            guard.insert(nonce_clone, pending_request);
        });

        tracing::info!(
            channel = %self.host_state.channel_name(),
            nonce = %nonce,
            "OAuth flow started, pending request registered"
        );

        Ok(near::agent::channel_host::OauthStartResult {
            url: auth_url,
            nonce,
        })
    }

    fn oauth_poll(&mut self, nonce: String) -> Result<String, String> {
        use crate::channels::wasm::router::OAuthCompletionResult;

        let completed_map = self.completed_oauth.as_ref().ok_or_else(|| {
            "OAuth poll not available: no completed OAuth store configured".to_string()
        })?;

        let pending_map = self.pending_oauth.as_ref().ok_or_else(|| {
            "OAuth poll not available: no pending OAuth store configured".to_string()
        })?;
        ...
        let rt = self.http_runtime.get_or_insert_with(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("failed to create tokio runtime for OAuth poll")
        });

        rt.block_on(async {
            // Check if the flow completed (success or failure)
            {
                let completed = completed_map.lock().await;
                if let Some(result) = completed.get(&nonce) {
                    return match result {
                        OAuthCompletionResult::Success { secret_name } => {
                            Ok(format!("complete:{}", secret_name))
                        }
                        OAuthCompletionResult::Failed { error } => Ok(format!("failed:{}", error)),
                    };
                }
            }

            // Check if a pending request still exists for this nonce
            {
            ...
            // Nonce not in pending or completed — unknown or expired
            Err(format!("Unknown or expired OAuth nonce: {}", nonce))
        })
    }
}
...
pub struct WasmChannel {
    ...
    /// Shared pending OAuth map for registering OAuth flows from WASM callbacks.
    pending_oauth: Option<
        Arc<
            tokio::sync::Mutex<HashMap<String, crate::channels::wasm::router::PendingOAuthRequest>>,
        >,
    ...
    /// Shared completed OAuth map for nonce-based polling by WASM channels.
    completed_oauth: Option<
        Arc<
            tokio::sync::Mutex<
                HashMap<String, crate::channels::wasm::router::OAuthCompletionResult>,
...
impl WasmChannel {
    ...
    /// Set the shared pending OAuth map, completed OAuth map, and redirect base for OAuth support.
    pub fn with_oauth(
        mut self,
        pending_oauth: Arc<
            tokio::sync::Mutex<HashMap<String, crate::channels::wasm::router::PendingOAuthRequest>>,
        ...
        self.pending_oauth = Some(pending_oauth);
        self.completed_oauth = Some(completed_oauth);
        self.oauth_redirect_base = Some(redirect_base);
        self
    }
    ...
    fn create_store(
        ...
        pending_oauth: Option<
            Arc<
                tokio::sync::Mutex<
                    HashMap<String, crate::channels::wasm::router::PendingOAuthRequest>,
                >,
            >,
        >,
        completed_oauth: Option<
        ...
        let mut store_data = ChannelStoreData::new(
            limits.memory_bytes,
            &prepared.name,
            capabilities.clone(),
            credentials,
        ...
        store_data.pending_oauth = pending_oauth;
        store_data.completed_oauth = completed_oauth;
        store_data.oauth_redirect_base = oauth_redirect_base;
    ...
    async fn call_on_start(&self) -> Result<ChannelConfig, WasmChannelError> {
        ...
        let pending_oauth = self.pending_oauth.clone();
        let completed_oauth = self.completed_oauth.clone();
        let oauth_redirect_base = self.oauth_redirect_base.clone();
        ...
        let result = tokio::time::timeout(timeout, async move {
            tokio::task::spawn_blocking(move || {
                let mut store = Self::create_store(
                    ...
                    pairing_store,
                    pending_oauth.clone(),
                    completed_oauth.clone(),
                    oauth_redirect_base.clone(),
                )?;
    ...
    match status {
        ...
        StatusUpdate::AgentStatus {
            ...
            message: format!(
                ...
                agent_id, memory_count, queue_depth
            ),
            metadata_json,
        },
    }
