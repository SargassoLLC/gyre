...
/// Maximum age for pending OAuth requests before they expire (10 minutes).
pub const PENDING_OAUTH_MAX_AGE_SECS: u64 = 600;

/// A pending OAuth authorization request awaiting callback.
#[derive(Debug, Clone)]
pub struct PendingOAuthRequest {
    /// Which WASM channel initiated this OAuth flow.
    pub channel_id: String,
    /// Token endpoint URL for exchanging code for token.
    pub token_url: String,
    /// OAuth client ID.
    pub client_id: String,
    /// Secret name containing the OAuth client secret.
    pub client_secret_name: String,
    /// The redirect URI registered with the OAuth provider.
    pub redirect_uri: String,
    /// Where to store the resulting access token in the secrets store.
...
/// Result of a completed OAuth flow, stored for nonce-based polling.
#[derive(Debug, Clone)]
pub enum OAuthCompletionResult {
    /// OAuth completed successfully. Contains the secret name where the token was stored.
    Success { secret_name: String },
    /// OAuth failed. Contains a human-readable error.
    Failed { error: String },
}
...
pub struct RouterState {
    ...
    pending_oauth: Arc<tokio::sync::Mutex<HashMap<String, PendingOAuthRequest>>>,
    /// Completed OAuth flows keyed by nonce, for polling by WASM channels.
    pub completed_oauth: Arc<tokio::sync::Mutex<HashMap<String, OAuthCompletionResult>>>,
    /// User ID for secrets store operations.
    user_id: String,
}
...
impl RouterState {
    pub fn new(router: Arc<WasmChannelRouter>) -> Self {
        Self {
            ...
            completed_oauth: Arc::new(tokio::sync::Mutex::new(HashMap::new())),
    ...
    /// Register a pending OAuth request and return the state nonce.
    #[allow(dead_code)]
    pub async fn register_oauth_request(&self, request: PendingOAuthRequest) -> String {
        let nonce = generate_oauth_nonce();
        let mut pending = self.pending_oauth.lock().await;

        // Lazy cleanup: remove expired entries
        pending.retain(|_, req| req.created_at.elapsed().as_secs() < PENDING_OAUTH_MAX_AGE_SECS);

        pending.insert(nonce.clone(), request);
        nonce
    }
}
...
/// Generate a cryptographically random OAuth state nonce (16 bytes, hex-encoded).
...
/// Looks up the state nonce to find the pending OAuth request, exchanges the
/// authorization code for an access token, and stores it in the secrets store.
#[allow(dead_code)]
async fn oauth_callback_handler(
    ...
    // Look up the pending OAuth request by state nonce
    let pending = {
        let mut map = state.pending_oauth.lock().await;
        // Lazy cleanup of expired entries
        map.retain(|_, req| req.created_at.elapsed().as_secs() < PENDING_OAUTH_MAX_AGE_SECS);
    ...
    let pending = match pending {
        ...
        None => {
            tracing::warn!(
                state = %oauth_state,
                "OAuth callback with unknown or expired state nonce"
            );
            return (
                StatusCode::BAD_REQUEST,
                axum::response::Html(
                    "<!DOCTYPE html><html><body style=\"font-family: sans-serif; \
                     ...
                     <div style=\"text-align: center;\">\
                     <h1>Authorization Failed</h1>\
                     <p>Unknown or expired OAuth state. Please try again.</p>\
                     </div></body></html>"
                        .to_string(),
                ),
            );
    ...
    let secrets = match &state.secrets {
        Some(s) => Arc::clone(s),
        None => {
            tracing::error!("OAuth callback received but no secrets store configured");
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                axum::response::Html(
                    "<!DOCTYPE html><html><body style=\"font-family: sans-serif; \
                     ...
                     <p>Secrets store not available.</p>\
                     </div></body></html>"
                        .to_string(),
                ),
            );
    ...
    // Get the client secret from the secrets store
    let client_secret = match secrets
        .get_decrypted(&state.user_id, &pending.client_secret_name)
        .await
    {
        Ok(decrypted) => decrypted.expose().to_string(),
        Err(e) => {
            tracing::error!(
                secret_name = %pending.client_secret_name,
                error = %e,
                "Failed to retrieve OAuth client secret"
            );
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                oauth_error_html(&format!(
                    "Failed to retrieve client secret '{}': {}",
                    pending.client_secret_name, e
                )),
            );
        }
    };
    ...
    let token_response = match token_response {
        ...
        Err(e) => {
            ...
            return (
                ...
                oauth_error_html(&format!("Token exchange request failed: {}", e)),
            );
        }
    };
    ...
    let token_body: serde_json::Value = match token_response.json().await {
        ...
        Err(e) => {
            ...
            return (
                StatusCode::BAD_GATEWAY,
                oauth_error_html(&format!("Failed to parse token response: {}", e)),
            );
        }
    };
    ...
    let access_token = match token_body.get("access_token").and_then(|v| v.as_str()) {
        Some(t) => t.to_string(),
        None => {
            tracing::error!(body = %token_body, "Token response missing access_token");
            return (
                StatusCode::BAD_GATEWAY,
                oauth_error_html("Token response missing access_token"),
            );
        }
    };

    // Store the access token
    let params = crate::secrets::CreateSecretParams::new(&pending.secret_name, &access_token)
        .with_provider(&pending.channel_id);

    if let Err(e) = secrets.create(&state.user_id, params).await {
        tracing::error!(
            secret_name = %pending.secret_name,
            error = %e,
            "Failed to store OAuth access token"
        );
        return (
            StatusCode::INTERNAL_SERVER_ERROR,
            oauth_error_html(&format!("Failed to store access token: {}", e)),
        );
    }
    ...
    tracing::info!(
        ...
        secret_name = %pending.secret_name,
        "OAuth token exchange completed, access token stored"
    );

    // Record completion so WASM channels can poll for the result
    {
        let mut completed = state.completed_oauth.lock().await;
        completed.insert(
            oauth_state.clone(),
            OAuthCompletionResult::Success {
                secret_name: pending.secret_name.clone(),
            },
        );
    }
    ...
    if let Some(refresh_token) = token_body.get("refresh_token").and_then(|v| v.as_str()) {
        let refresh_name = format!("{}_refresh", pending.secret_name);
        let refresh_params = crate::secrets::CreateSecretParams::new(&refresh_name, refresh_token)
            .with_provider(&pending.channel_id);

        if let Err(e) = secrets.create(&state.user_id, refresh_params).await {
            tracing::warn!(
                secret_name = %refresh_name,
                error = %e,
                "Failed to store OAuth refresh token (non-fatal)"
            );
        } else {
            tracing::info!(
                secret_name = %refresh_name,
                "OAuth refresh token stored"
            );
        }
    }
    ...
    (
        ...
        axum::response::Html(
            "<!DOCTYPE html><html><body style=\"font-family: sans-serif; \
             ...
             <div style=\"text-align: center;\">\
             <h1>&#x2705; Connected!</h1>\
             <p>You can close this window and return to Gyre.</p>\
             </div></body></html>"
                .to_string(),
        ),
    )
...
/// Build an HTML error page for OAuth failures.
fn oauth_error_html(detail: &str) -> axum::response::Html<String> {
    axum::response::Html(format!(
        "<!DOCTYPE html><html><body style=\"font-family: sans-serif; \
         display: flex; justify-content: center; align-items: center; \
         height: 100vh; margin: 0; background: #191919; color: white;\">\
...
pub fn create_wasm_channel_router(
    ...
    if let Some(secrets) = secrets {
        state = state.with_secrets(secrets, user_id.unwrap_or_else(|| "default".to_string()));
    }
