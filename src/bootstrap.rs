...
//!
//! File: `~/.gyre/.env` (standard dotenvy format)

use std::path::PathBuf;

/// Path to the Gyre-specific `.env` file: `~/.gyre/.env`.
pub fn gyre_env_path() -> PathBuf {
    dirs::home_dir()
...
/// Load env vars from `~/.gyre/.env` (in addition to the standard `.env`).
///
/// Call this **after** `dotenvy::dotenv()` so that the standard `./.env`
/// takes priority over `~/.gyre/.env`. dotenvy never overwrites
/// existing env vars, so the effective priority is:
///
///   explicit env vars > `./.env` > `~/.gyre/.env`
///
/// If `~/.gyre/.env` doesn't exist but the legacy `bootstrap.json` does,
/// extracts `DATABASE_URL` from it and writes the `.env` file (one-time
/// upgrade from the old config format).
pub fn load_gyre_env() {
    let path = gyre_env_path();
...
fn migrate_bootstrap_json_to_env(env_path: &std::path::Path) {
    let gyre_dir = env_path
        .parent()
        .unwrap_or_else(|| std::path::Path::new("."));
    let bootstrap_path = gyre_dir.join("bootstrap.json");
...
/// Write database bootstrap vars to `~/.gyre/.env`.
///
...
/// Write `DATABASE_URL` to `~/.gyre/.env`.
///
...
/// One-time migration of legacy `~/.gyre/settings.json` into the database.
///
...
pub async fn migrate_disk_to_db(
    ...
    let gyre_dir = dirs::home_dir()
        .unwrap_or_else(|| PathBuf::from("."))
        .join(".gyre");
    let legacy_settings_path = gyre_dir.join("settings.json");
    ...
    let session_path = gyre_dir.join("session.json");
    if session_path.exists() {
        match std::fs::read_to_string(&session_path) {
            Ok(content) => match serde_json::from_str::<serde_json::Value>(&content) {
                Ok(value) => {
                    store
                        .set_setting(user_id, "gyre.session_token", &value)
                        .await
                        .map_err(|e| {
                            MigrationError::Database(format!(
    ...
    let old_bootstrap = gyre_dir.join("bootstrap.json");
...
mod tests {
    ...
    fn test_save_and_load_database_url() {
        ...
        let url = "postgres://localhost:5432/gyre_test";
        ...
        assert_eq!(
            content,
            "DATABASE_URL=\"postgres://localhost:5432/gyre_test\"\n"
        );
    ...
    fn test_save_database_url_with_hash_in_password() {
        ...
        let url = "postgres://user:p%23ss@localhost:5432/gyre";
        std::fs::write(&env_path, format!("DATABASE_URL=\"{}\"\n", url)).unwrap();
...
INJECTED="pwned"#;
    ...
    #[test]
    fn test_gyre_env_path() {
        let path = gyre_env_path();
        assert!(path.ends_with(".gyre/.env"));
    }
    ...
    fn test_migrate_bootstrap_json_to_env() {
        ...
        let bootstrap_json = serde_json::json!({
            "database_url": "postgres://localhost/gyre_upgrade",
    ...
    fn test_save_bootstrap_env_multiple_vars() {
        ...
        let vars = [
            ...
            ("LIBSQL_PATH", "/home/user/.gyre/gyre.db"),
        ];
        ...
        assert_eq!(
            ...
            (
                ...
                "/home/user/.gyre/gyre.db".to_string()
            )
        );
    }
