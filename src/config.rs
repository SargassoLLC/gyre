//! Configuration for Gyre.
//!
//! Settings are loaded with priority: env var > database > default.
//! `DATABASE_URL` lives in `~/.gyre/.env` (loaded via dotenvy early
...
impl Config {
    ...
    ///
    /// Loads both `./.env` (standard, higher priority) and `~/.gyre/.env`
    ...
    /// If `None`, tries the default path `~/.gyre/config.toml` (missing
    ...
    async fn build(settings: &Settings) -> Result<Self, ConfigError> {
        Ok(Self {
            ...
            resilience: ResilienceConfig::resolve(settings)?,
        })
    }
}
...
pub enum DatabaseBackend {
    ...
    Postgres,
    /// libSQL/Turso embedded database (default — zero config, creates ~/.gyre/gyre.db).
    #[default]
    LibSql,
}
...
pub struct DatabaseConfig {
    ...
    // -- libSQL fields --
    /// Path to local libSQL database file (default: ~/.gyre/gyre.db).
...
impl DatabaseConfig {
    fn resolve() -> Result<Self, ConfigError> {
        ...
        // DATABASE_URL is loaded from ~/.gyre/.env via dotenvy early in startup.
        let url = optional_env("DATABASE_URL")?
            ...
            .ok_or_else(|| ConfigError::MissingRequired {
                ...
                hint: "Run 'gyre onboard' or set DATABASE_URL environment variable".to_string(),
            })?;
...
/// Default libSQL database path (~/.gyre/gyre.db).
...
pub enum LlmBackend {
    /// Direct OpenAI API
    OpenAi,
    /// Direct Anthropic API (default — auto-detects Claude Code OAuth)
    #[default]
    Anthropic,
...
impl LlmConfig {
    fn resolve(settings: &Settings) -> Result<Self, ConfigError> {
        // Determine backend: env var > settings > default (Anthropic)
        let backend: LlmBackend = if let Some(b) = optional_env("LLM_BACKEND")? {
            ...
            match b.parse() {
                ...
                Err(e) => {
                    tracing::warn!(
                        "Invalid llm_backend '{}' in settings: {}. Using default Anthropic.",
                        b,
                        e
                    );
                    LlmBackend::OpenAi
        ...
        let anthropic = if backend == LlmBackend::Anthropic {
            // Priority: ANTHROPIC_API_KEY env var > Claude Code OAuth token (keychain)
            let api_key = if let Some(env_key) = optional_env("ANTHROPIC_API_KEY")? {
                SecretString::from(env_key)
            } else if let Some(oauth_token) = ClaudeCodeConfig::extract_oauth_token() {
...
pub struct EmbeddingsConfig {
    ...
    pub enabled: bool,
    /// Provider to use: "openai" or "gyre"
    pub provider: String,
...
pub struct ChannelsConfig {
    ...
    /// Directory containing WASM channel modules (default: ~/.gyre/channels/).
...
/// Get the default channels directory (~/.gyre/channels/).
...
pub struct WasmConfig {
    ...
    pub enabled: bool,
    /// Directory containing installed WASM tools (default: ~/.gyre/tools/).
...
/// Get the default tools directory (~/.gyre/tools/).
...
impl Default for SandboxModeConfig {
    fn default() -> Self {
        Self {
            ...
            cpu_shares: 1024,
            image: "ghcr.io/sargassollc/gyre-sandbox:latest".to_string(),
    ...
    fn resolve() -> Result<Self, ConfigError> {
        ...
        Ok(Self {
            ...
            image: optional_env("SANDBOX_IMAGE")?
                .unwrap_or_else(|| "ghcr.io/sargassollc/gyre-sandbox:latest".to_string()),
...
pub struct SkillsConfig {
    ...
    pub enabled: bool,
    /// Directory containing local skills (default: ~/.gyre/skills/).
...
/// Get the default skills directory (~/.gyre/skills/).
...
/// Resilience configuration (circuit breaker, response cache, failover).
#[derive(Debug, Clone)]
pub struct ResilienceConfig {
    ...
    pub failover_fallback_backend: Option<String>,
    pub failover_fallback_model: Option<String>,
    pub failover_fallback_base_url: Option<String>,
    pub failover_fallback_api_key: Option<SecretString>,
    pub failover_cooldown_secs: u64,
}

impl ResilienceConfig {
    fn resolve(settings: &Settings) -> Result<Self, ConfigError> {
        let rs = &settings.resilience;

        Ok(Self {
            circuit_breaker_enabled: optional_env("RESILIENCE_CIRCUIT_BREAKER_ENABLED")?
                .map(|s| s.parse())
                .transpose()
                .map_err(|e| ConfigError::InvalidValue {
                    key: "RESILIENCE_CIRCUIT_BREAKER_ENABLED".to_string(),
                    message: format!("{e}"),
                })?
                .unwrap_or(rs.circuit_breaker.enabled),
            circuit_breaker_failure_threshold: parse_optional_env(
                "RESILIENCE_CIRCUIT_BREAKER_FAILURE_THRESHOLD",
                rs.circuit_breaker.failure_threshold,
            )?,
            circuit_breaker_reset_timeout_secs: parse_optional_env(
                "RESILIENCE_CIRCUIT_BREAKER_RESET_TIMEOUT_SECS",
                rs.circuit_breaker.reset_timeout_secs,
            )?,

            response_cache_enabled: optional_env("RESILIENCE_RESPONSE_CACHE_ENABLED")?
                .map(|s| s.parse())
                .transpose()
                .map_err(|e| ConfigError::InvalidValue {
                    key: "RESILIENCE_RESPONSE_CACHE_ENABLED".to_string(),
                    message: format!("{e}"),
                })?
                .unwrap_or(rs.response_cache.enabled),
            response_cache_ttl_secs: parse_optional_env(
                "RESILIENCE_RESPONSE_CACHE_TTL_SECS",
                rs.response_cache.ttl_secs,
            )?,
            response_cache_max_entries: parse_optional_env(
                "RESILIENCE_RESPONSE_CACHE_MAX_ENTRIES",
                rs.response_cache.max_entries,
            )?,

            failover_enabled: optional_env("RESILIENCE_FAILOVER_ENABLED")?
                .map(|s| s.parse())
                .transpose()
                .map_err(|e| ConfigError::InvalidValue {
                    key: "RESILIENCE_FAILOVER_ENABLED".to_string(),
                    message: format!("{e}"),
                })?
                .unwrap_or(rs.failover.enabled),
            failover_fallback_backend: optional_env("RESILIENCE_FAILOVER_FALLBACK_BACKEND")?
                .or_else(|| rs.failover.fallback_backend.clone()),
            failover_fallback_model: optional_env("RESILIENCE_FAILOVER_FALLBACK_MODEL")?
                .or_else(|| rs.failover.fallback_model.clone()),
            failover_fallback_base_url: optional_env("RESILIENCE_FAILOVER_FALLBACK_BASE_URL")?
                .or_else(|| rs.failover.fallback_base_url.clone()),
            failover_fallback_api_key: optional_env("RESILIENCE_FAILOVER_FALLBACK_API_KEY")?
                .or_else(|| rs.failover.fallback_api_key.clone())
                .map(SecretString::from),
            failover_cooldown_secs: parse_optional_env(
                "RESILIENCE_FAILOVER_COOLDOWN_SECS",
                rs.failover.cooldown_secs,
            )?,
        })
    }
}
