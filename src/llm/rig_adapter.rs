//! Generic adapter that bridges rig-core's `CompletionModel` trait to Gyre's `LlmProvider`.
//!
...
pub struct RigAdapter<M: CompletionModel> {
    ...
    input_cost: Decimal,
    output_cost: Decimal,
    /// Optional prefix prepended to the system prompt (preamble).
    /// Used by OAuth providers that require a Claude Code identity header.
...
impl<M: CompletionModel> RigAdapter<M> {
    ...
    /// Set a prefix that will be prepended to the system prompt in every request.
    pub fn with_preamble_prefix(mut self, prefix: impl Into<String>) -> Self {
        self.preamble_prefix = Some(prefix.into());
        self
    }

    /// Prepend the configured prefix to the preamble, if set.
    fn apply_preamble_prefix(&self, preamble: Option<String>) -> Option<String> {
        match (&self.preamble_prefix, preamble) {
            (Some(prefix), Some(existing)) => Some(format!("{}\n{}", prefix, existing)),
            (Some(prefix), None) => Some(prefix.clone()),
            (None, preamble) => preamble,
        }
    }
}
...
/// Convert Gyre messages to rig-core format.
///
...
/// Convert Gyre tool definitions to rig-core format.
...
/// Convert Gyre tool_choice string to rig-core ToolChoice.
...
fn extract_response(
    ...
    for content in choice.iter() {
        match content {
            ...
            // Reasoning and Image variants are not mapped to Gyre types
            _ => {}
        }
    }
    ...
    M::Response: Send + Sync + Serialize + DeserializeOwned,
{
    ...
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse, LlmError> {
        ...
        let preamble = self.apply_preamble_prefix(preamble);
