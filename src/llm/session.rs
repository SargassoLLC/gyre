//! Session management for Gyre authentication.
//!
//! Handles session token persistence, expiration detection, and renewal via
//! OAuth flow. Tokens are stored in `~/.gyre/session.json` and refreshed
...
pub struct SessionConfig {
    /// Base URL for auth endpoints (override in config if needed).
    pub auth_base_url: String,
    /// Path to session file (e.g., ~/.gyre/session.json).
...
impl Default for SessionConfig {
    fn default() -> Self {
        Self {
            auth_base_url: "https://auth.getgyre.com".to_string(),
            session_path: default_session_path(),
        }
    }
}

/// Get the default session file path (~/.gyre/session.json).
...
/// Manages Gyre session tokens with persistence and automatic renewal.
...
impl SessionManager {
    ...
    /// table (key: `gyre.session_token`) in addition to the disk file.
    ...
    async fn initiate_login(&self) -> Result<(), LlmError> {
        ...
        // The Gyre API redirects to: {frontend_callback}/auth/callback?token=X&...
        let session_token =
            oauth_defaults::wait_for_callback(listener, "/auth/callback", "token", "Gyre")
                .await
                .map_err(|e| LlmError::SessionRenewalFailed {
                    provider: "gyre".to_string(),
                    reason: e.to_string(),
    ...
    async fn save_session(&self, token: &str, auth_provider: Option<&str>) -> Result<(), LlmError> {
        ...
        if let Some(ref store) = *self.store.read().await {
            ...
            if let Err(e) = store
                .set_setting(&user_id, "gyre.session_token", &session_json)
                .await
            {
    ...
    async fn load_session_from_db(&self) -> Result<(), LlmError> {
        ...
        let value = if let Some(value) = store
            .get_setting(&user_id, "gyre.session_token")
            .await
            .map_err(|e| LlmError::SessionRenewalFailed {
                provider: "gyre".to_string(),
            ...
            tracing::warn!(
                "gyre.session_token missing; falling back to legacy gyre.session for backwards compatibility"
            );
            store
                .get_setting(&user_id, "gyre.session")
                .await
                .map_err(|e| LlmError::SessionRenewalFailed {
                    provider: "gyre".to_string(),
                    reason: format!("DB query failed: {}", e),
...
pub async fn create_session_manager(config: SessionConfig) -> Arc<SessionManager> {
    ...
    if !manager.has_token().await
        && let Ok(token) = std::env::var("GYRE_SESSION_TOKEN")
        && !token.is_empty()
    {
        tracing::info!("Migrating session token from GYRE_SESSION_TOKEN env var to file");
