...
//!
//! Every OAuth flow in the codebase (WASM tool auth, MCP server auth, Gyre login)
...
//!
//! - **Compile time**: Set GYRE_GOOGLE_CLIENT_ID / GYRE_GOOGLE_CLIENT_SECRET
...
const GOOGLE_CLIENT_ID: &str = match option_env!("GYRE_GOOGLE_CLIENT_ID") {
    Some(v) => v,
    None => "REDACTED_GOOGLE_CLIENT_ID",
};
const GOOGLE_CLIENT_SECRET: &str = match option_env!("GYRE_GOOGLE_CLIENT_SECRET") {
    Some(v) => v,
...
/// `http://localhost:9876/callback` (or `/auth/callback` for Gyre).
...
/// explicitly (e.g., Gyre redirects to `http://127.0.0.1:9876/auth/callback`).
...
/// landing page using `display_name` (e.g., "Google", "Notion", "Gyre").
///
