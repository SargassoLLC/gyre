...
async fn add_server(
    ...
    if requires_auth {
        println!();
        println!("  Run 'gyre mcp auth {}' to authenticate.", name);
    }
...
async fn list_servers(verbose: bool) -> anyhow::Result<()> {
    ...
    if servers.servers.is_empty() {
        ...
        println!("    gyre mcp add <name> <url> [--client-id <id>]");
        println!();
        return Ok(());
    }
...
async fn auth_server(name: String, user_id: String) -> anyhow::Result<()> {
    ...
    match authorize_mcp_server(&server, &secrets, &user_id).await {
        ...
        Err(crate::tools::mcp::auth::AuthError::NotSupported) => {
            ...
            println!();
            println!("    gyre mcp remove {}", name);
            println!(
                "    gyre mcp add {} {} --client-id YOUR_CLIENT_ID",
                name, server.url
            );
            println!();
        }
...
async fn test_server(name: String, user_id: String) -> anyhow::Result<()> {
    ...
    let client = if has_tokens {
        ...
        println!();
        println!("  ✗ Not authenticated. Run 'gyre mcp auth {}' first.", name);
        println!();
        return Ok(());
    } else {
