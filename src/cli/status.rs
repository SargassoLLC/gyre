...
pub async fn run_status_command() -> anyhow::Result<()> {
    ...
    if session_path.exists() {
        ...
        println!("not found (run `gyre onboard`)");
    }
    ...
    let resilience_info = {
        ...
        if rs.circuit_breaker.enabled {
            parts.push(format!(
                "circuit-breaker(threshold:{}, reset:{}s)",
                rs.circuit_breaker.failure_threshold, rs.circuit_breaker.reset_timeout_secs
            ));
        }
        if rs.response_cache.enabled {
            parts.push(format!(
                "cache(ttl:{}s, max:{})",
                rs.response_cache.ttl_secs, rs.response_cache.max_entries
            ));
        }
        if rs.failover.enabled {
            let backend = rs.failover.fallback_backend.as_deref().unwrap_or("none");
            parts.push(format!("failover({})", backend));
        }
        if parts.is_empty() {
            "disabled".to_string()
        } else {
    ...
    println!(
        "\n  Config:      {}",
        crate::bootstrap::gyre_env_path().display()
    );
