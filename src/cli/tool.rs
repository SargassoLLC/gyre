...
fn default_tools_dir() -> PathBuf {
    dirs::home_dir()
        .map(|h| h.join(".gyre").join("tools"))
        .unwrap_or_else(|| PathBuf::from(".gyre/tools"))
}
...
pub enum ToolCommand {
    ...
    Install {
        ...
        /// Target directory for installation (default: ~/.gyre/tools/)
        #[arg(short, long)]
    ...
    List {
        /// Directory to list tools from (default: ~/.gyre/tools/)
        #[arg(short, long)]
        dir: Option<PathBuf>,
    ...
    Remove {
        ...
        /// Directory to remove tool from (default: ~/.gyre/tools/)
        #[arg(short, long)]
        dir: Option<PathBuf>,
    },
    ...
    Info {
        ...
        /// Directory to look for tool (default: ~/.gyre/tools/)
        #[arg(short, long)]
        dir: Option<PathBuf>,
    },
...
async fn list_tools(dir: Option<PathBuf>, verbose: bool) -> anyhow::Result<()> {
    ...
    if !tools_dir.exists() {
        ...
        println!("Install a tool with: gyre tool install <path>");
        return Ok(());
    }
...
async fn auth_tool(name: String, dir: Option<PathBuf>, user_id: String) -> anyhow::Result<()> {
    ...
    let master_key = config.secrets.master_key().ok_or_else(|| {
        anyhow::anyhow!("SECRETS_MASTER_KEY not set. Run 'gyre onboard' first or set it in .env")
    })?;
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "OAuth client_id not configured.\n\
                 Set {} env var, or build with GYRE_GOOGLE_CLIENT_ID.",
...
mod tests {
    ...
    fn test_default_tools_dir() {
        ...
        assert!(dir.to_string_lossy().contains(".gyre"));
