//! OS service management for running Gyre as a daemon.
//!
//! Generates and manages platform-native service definitions:
//! - **macOS**: launchd plist at `~/Library/LaunchAgents/com.gyre.daemon.plist`
//! - **Linux**: systemd user unit at `~/.config/systemd/user/gyre.service`
//!
//! The installed service runs `gyre run` (the default agent mode) and is
...
const SERVICE_LABEL: &str = "com.gyre.daemon";
const SYSTEMD_UNIT: &str = "gyre.service";
...
"#,
    ...
    println!("  Start with: gyre service start");
    Ok(())
}
...
fn start() -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        if !plist.exists() {
            bail!("Service not installed. Run `gyre service install` first.");
        }
...
fn gyre_logs_dir() -> Result<PathBuf> {
    let home = dirs::home_dir().context("could not find home directory")?;
    Ok(home.join(".gyre").join("logs"))
}
...
mod tests {
    ...
    fn macos_plist_path_has_expected_suffix() {
        ...
        assert!(
            s.ends_with("Library/LaunchAgents/com.gyre.daemon.plist"),
            "unexpected path: {s}"
        );
    }
    ...
    fn linux_unit_path_has_expected_suffix() {
        ...
        let s = path.to_string_lossy();
        assert!(
            s.ends_with(".config/systemd/user/gyre.service"),
            "unexpected path: {s}"
        );
    }

    #[test]
    fn logs_dir_under_gyre() {
        let path = gyre_logs_dir().unwrap();
        let s = path.to_string_lossy();
        assert!(s.ends_with(".gyre/logs"), "unexpected path: {s}");
    }
}
