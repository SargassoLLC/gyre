...
//! loader.load_from_dir(Path::new("~/.gyre/tools/")).await?;
//! ```
//!
//! # Dev Mode
//!
...
/// needing to install them to `~/.gyre/tools/` first. Build artifacts
/// that are newer than installed copies take priority.
///
/// Set `GYRE_TOOLS_SRC` env var to override the source directory.
