...
use crate::tools::wasm::wit_schema;
...
impl WasmToolRuntime {
    ...
    pub async fn prepare(
        ...
        let prepared = tokio::task::spawn_blocking(move || {
            ...
            // Extract description and schema by instantiating with stub host
            // functions and calling the tool's description() and schema() exports.
            let (description, schema) =
                match wit_schema::extract_wit_schema(&engine, &component, &name) {
                    Some(ref wit) => {
                        let desc = wit
                            .description
                            .clone()
                            .unwrap_or_else(|| "WASM sandboxed tool".to_string());
                        let sch = wit_schema::wit_schema_to_json_schema(wit);
                        (desc, sch)
                    }
                    None => {
                        // Fallback: tool didn't export expected interface
                        tracing::debug!(
                            name = %name,
                            "WIT schema extraction returned None, using defaults"
                        );
                        (
