        ...
        for (secret_name, env_var_value) in creds_obj {
            ...
            if !exists {
                return Err(ToolError::ExecutionFailed(format!(
                    "secret '{}' not found. Store it first via 'gyre tool auth' or the web UI.",
                    secret_name
                )));
            }
...
///
/// Auto-creates `~/.gyre/projects/{project_id}/` so every sandbox job has a
...
/// it is validated to fall within `~/.gyre/projects/` after canonicalization.
            ...
            serde_json::json!({
                ...
                "properties": {
                    ...
                    "mode": {
                        ...
                        "description": "Execution mode. 'worker' (default) uses the Gyre sub-agent. \
                    ...
                    "project_dir": {
                        ...
                        "description": "Path to an existing project directory to mount into the container. \
                                        Must be under ~/.gyre/projects/. If omitted, a fresh directory is created."
                    },
                    "credentials": {
                        "type": "object",
...
mod tests {
    ...
    async fn test_parse_credentials_valid() {
        ...
        secrets
            .create(
                "user1",
                CreateSecretParams::new("github_token", "test_pat_placeholder_123"),
            )
            .await
            .unwrap();
