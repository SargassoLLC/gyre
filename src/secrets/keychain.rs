...
//!
//! ```ignore
//! use gyre::secrets::keychain::{store_master_key, get_master_key, delete_master_key};
//!
...
mod platform {
    ...
    pub async fn get_master_key() -> Result<Vec<u8>, SecretError> {
        // Run in a blocking task with a timeout so headless/non-TTY processes
        // don't hang forever waiting for a macOS GUI auth dialog.
        let result = tokio::time::timeout(
            std::time::Duration::from_secs(3),
            tokio::task::spawn_blocking(|| get_generic_password(SERVICE_NAME, MASTER_KEY_ACCOUNT)),
        )
        .await
        .map_err(|_| {
            SecretError::KeychainError("Keychain access timed out (no GUI available)".to_string())
        })?
        .map_err(|e| SecretError::KeychainError(format!("spawn_blocking failed: {}", e)))?
        .map_err(|e| SecretError::KeychainError(format!("Failed to get from keychain: {}", e)))?;

        // Parse hex string back to bytes
        let hex_str = String::from_utf8(result)
