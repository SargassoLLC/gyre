...
3. Set up Interactions URL to point to your Gyre instance
4. Copy the Application ID and Public Key
5. Store in Gyre secrets:

   ```bash
   gyre secret set discord_bot_token YOUR_BOT_TOKEN
   ```
   ...
   **Note:** The `discord_bot_token` secret is the only value read directly by this
   ...
   secrets are used by the Gyre host (for example, to verify Discord
...
- Interactions Endpoint URL: `https://your-gyre.com/webhook/discord`
...
- Check that `discord_public_key` is set correctly in Gyre secrets.
- This validation happens on the host before reaching the WASM.

### "401 Unauthorized"

- Check that `discord_bot_token` is set correctly in Gyre secrets.
