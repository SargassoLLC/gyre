...
This guide covers configuring the Telegram channel for Gyre, including DM pairing for access control.

## Overview

The Telegram channel lets you interact with Gyre via Telegram DMs and groups. It supports:
...
2. Bot replies: `To pair with this bot, run: gyre pairing approve telegram ABC12345`
3. You run: `gyre pairing approve telegram ABC12345`
...
gyre pairing list telegram

# List as JSON
gyre pairing list telegram --json

# Approve a user by code
gyre pairing approve telegram ABC12345
```

### Configuration

Edit `~/.gyre/channels/telegram.capabilities.json` (or the config injected by the host):
...
- Set `bot_username` in config to your bot's username (e.g., `MyGyreBot`)
- Ensure the message contains `@YourBot` or starts with `/`

### "Connection refused" when starting

- For webhook mode: Start your tunnel before `gyre run`
