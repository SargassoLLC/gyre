...
This guide covers how to build WASM channel modules for Gyre.
...
version = "0.1.0"
edition = "2021"
description = "My messaging platform channel for Gyre"
...
**Do not commit compiled WASM binaries.** They are a supply chain risk — the binary in a PR may not match the source. Gyre builds channels from source:
...
# Install (or use gyre onboard to install bundled channel)
mkdir -p ~/.gyre/channels
cp channels-src/telegram/telegram.wasm channels-src/telegram/telegram.capabilities.json ~/.gyre/channels/
```

**Note**: The main Gyre binary bundles `telegram.wasm` via `include_bytes!`. When modifying the Telegram channel source, run `./channels-src/telegram/build.sh` **before** building the main crate, so the updated WASM is included.
...
# Deploy to ~/.gyre/channels/
cp target/wasm32-wasip2/release/my_channel.wasm ~/.gyre/channels/my-channel.wasm
cp my-channel.capabilities.json ~/.gyre/channels/
```
