---
description: Deep audit of the Gyre crate for vulnerabilities, bugs, unfinished work, inconsistencies, and oversights
...
Gyre convention: `.unwrap()` and `.expect()` are banned in production code. Any occurrence outside `#[cfg(test)]` blocks is a **High severity** finding.
...
Gyre has two database backends (PostgreSQL and libSQL). Check both for injection vectors.
...
- Is `thiserror` used consistently for error types (Gyre convention)?
...
Gyre convention: use `crate::` imports, not `super::`. Flag any `super::` imports in non-test code.
...
Gyre key traits: `Database` (~60 methods), `Channel`, `Tool`, `LlmProvider`, `SuccessEvaluator`, `EmbeddingProvider`. If any new methods were added to `Database`, verify both `postgres.rs` and `libsql_backend.rs` implement them.
