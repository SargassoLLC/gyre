---
description: Trace a data flow or bug through the Gyre codebase end-to-end
allowed-tools: Read, Glob, Grep, Bash(cargo test:*)
argument-hint: <symptom or feature name>
model: sonnet
---

Trace the flow of `$ARGUMENTS` through the Gyre codebase. Your job is to map every file and function involved, identify where data transforms or could break, and report the full chain.

## Architecture Reference

Gyre has three main data flow paths. Identify which one(s) are relevant and trace through them:
  ...
  → Agent::run() message loop (agent_loop.rs)
    ...
    → run_agentic_loop() iterates LLM calls
      → Reasoning::respond_with_tools() (reasoning.rs)
        → LlmProvider::complete_with_tools() (llm_provider.rs)
...
Tool trait impl (tools/builtin/*.rs or tools/mcp/client.rs or tools/wasm/wrapper.rs)
  ...
  → tool_definitions() builds Vec<ToolDefinition> for LLM
    ...
    → Serialized to ChatCompletionTool
...
| Chat completions | `src/llm/mod.rs` | `complete_with_tools`, `From<ChatMessage>` |
| Responses API | `src/llm/mod.rs` | `complete_with_tools`, `split_messages` |
