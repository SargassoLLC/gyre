...
A standalone WASM component that provides Slack integration for Gyre. This serves as both a functional tool and a template for building custom WASM tools.
...
```bash
mkdir -p ~/.gyre/tools
cp target/wasm32-wasip2/release/slack_tool.wasm ~/.gyre/tools/slack.wasm
cp slack.capabilities.json ~/.gyre/tools/
```
...
```bash
gyre secret set slack_bot_token "xoxb-your-token-here"
```
...
```bash
gyre secret set slack_bot_token "xoxb-..."
```
