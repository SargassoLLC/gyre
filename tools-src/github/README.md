...
3. Store the token:
   ...
   ```
   gyre secret set github_token YOUR_TOKEN
   ```
...
```text
GitHub token not found in secret store. Set it with: gyre secret set github_token <token>...
```
...
- Update the token: `gyre secret set github_token NEW_TOKEN`.
